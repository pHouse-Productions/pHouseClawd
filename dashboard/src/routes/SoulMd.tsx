...
export default function SoulMd() {
    ...
    const fetchContent = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setContent(data.soulMd || "");
        }
      } catch (err) {
        console.error("Failed to fetch SOUL.md:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await authFetch("/api/config", {
        ...
        body: JSON.stringify({ type: "soulMd", data: content }),
      });
      const data = await res.json();
      if (res.ok) {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-5.5rem)] md:h-[calc(100vh-3rem)]">
      <div className="flex items-center justify-between mb-4">
        <div>
          <Link to="/system" className="text-zinc-400 hover:text-white text-sm">
            ← Back to System
          </Link>
