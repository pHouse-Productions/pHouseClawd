...
async function getDocumentLink(documentId: string): Promise<string> {
...
async function readDocument(documentId: string): Promise<{ title: string; content: string }> {
  const response = await docs.documents.get({
    documentId,
  });
...
async function appendToDocument(documentId: string, text: string): Promise<void> {
  ...
  tools: [
    {
      ...
      description:
        "Create a new Google Doc with an optional title and content. Returns the document ID and URL.",
      inputSchema: {
        type: "object" as const,
        properties: {
          title: {
            type: "string",
            description: "The title of the document (default: 'Untitled')",
          },
          content: {
            type: "string",
    ...
    },
    {
      name: "share_document",
      description:
        "Share a Google Doc with an email address. Defaults to writer (edit) access.",
      inputSchema: {
        type: "object" as const,
        properties: {
          document_id: {
          ...
          role: {
            ...
            description: "Permission level: reader, commenter, or writer (default: writer)",
          },
        },
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  if (name === "create_document") {
    const { title = "Untitled", content } = (args as {
      title?: string;
      content?: string;
    }) || {};
    ...
    try {
      ...
      return {
        content: [{ type: "text", text: `Failed to create document: ${errMsg}` }],
        isError: true,
      };
    }
  }
  ...
  if (name === "share_document") {
    ...
    try {
      ...
      const errMsg = error instanceof Error ? error.message : String(error);
      return {
        content: [{ type: "text", text: `Failed to share document: ${errMsg}` }],
        isError: true,
      };
    }
  }
  ...
  if (name === "get_document_link") {
    ...
    try {
      const url = await getDocumentLink(document_id);
      return {
      ...
      const errMsg = error instanceof Error ? error.message : String(error);
      return {
        content: [{ type: "text", text: `Failed to get document link: ${errMsg}` }],
        isError: true,
      };
    }
  }
  ...
  if (name === "read_document") {
    ...
    try {
      ...
      const errMsg = error instanceof Error ? error.message : String(error);
      return {
        content: [{ type: "text", text: `Failed to read document: ${errMsg}` }],
        isError: true,
      };
    }
  }
    ...
    try {
      ...
      return {
        content: [
          {
            type: "text",
            text: `Successfully appended text to document ${document_id}`,
          },
        ],
      };
    } catch (error) {
      const errMsg = error instanceof Error ? error.message : String(error);
      return {
        content: [{ type: "text", text: `Failed to append to document: ${errMsg}` }],
        isError: true,
      };
    }
  }
...
async function main() {
  ...
  console.error("[MCP] Google Docs MCP server running");
}
