...
# Kill by specific command patterns (most reliable)
pkill -f "tsx integrations/telegram/src/receive.ts" 2>/dev/null
pkill -f "tsx integrations/gmail/src/receive.ts" 2>/dev/null
...
REMAINING=$(pgrep -f "tsx.*(telegram|gmail|watcher)" 2>/dev/null)

if [ -z "$REMAINING" ]; then
    echo "All pHouseClawd processes killed."
else
    echo "Some processes still running, force killing..."
    pkill -9 -f "tsx integrations/telegram/src/receive.ts" 2>/dev/null
    pkill -9 -f "tsx integrations/gmail/src/receive.ts" 2>/dev/null
